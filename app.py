    ...
    if 'loggedin' in session:
        ...
        if request.method == 'POST' and 'password' in request.form and 'confirm_pass' in request.form and 'userid' in request.form:
            ...
            if not password or not confirm_pass:
                ...
                cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
                cursor.execute(
                    'UPDATE user SET  password =% s WHERE id =% s', (password, (userId, ), ))
                mysql.connection.commit()
...
def delete_issue_book():
    if 'loggedin' in session:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'DELETE FROM issued_book WHERE issuebookid = % s', (issuebookid, ))
        mysql.connection.commit()
        return redirect(url_for('list_issue_book'))
    return redirect(url_for('login'))
    ...
    if 'loggedin' in session:
        ...
        if request.method == 'POST' and 'name' in request.form and 'status' in request.form:
            ...
            if action == 'updateCategory':
                ...
                cursor.execute(
                    'UPDATE category SET name = %s, status = %s WHERE categoryid =% s', (name, status, (categoryId, ), ))
                mysql.connection.commit()
            else:
                cursor.execute(
                    'INSERT INTO category (`name`, `status`) VALUES (%s, %s)', (name, status))
                mysql.connection.commit()
...
def editCategory():
    if 'loggedin' in session:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'SELECT categoryid, name, status FROM category WHERE categoryid = %s', (categoryid,))
        categories = cursor.fetchall()
    ...
    if 'loggedin' in session:
        ...
        if request.method == 'POST' and 'name' in request.form and 'status' in request.form:
            ...
            if action == 'updateAuthor':
                ...
                cursor.execute(
                    'UPDATE author SET name = %s, status = %s WHERE authorid =% s', (name, status, (authorId, ), ))
                mysql.connection.commit()
            else:
                cursor.execute(
                    'INSERT INTO author (`name`, `status`) VALUES (%s, %s)', (name, status))
                mysql.connection.commit()
...
def editAuthor():
    if 'loggedin' in session:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'SELECT authorid, name, status FROM author WHERE authorid = %s', (authorid,))
        authors = cursor.fetchall()
    ...
    if 'loggedin' in session:
        ...
        if request.method == 'POST' and 'name' in request.form and 'status' in request.form:
            ...
            if action == 'updatePublisher':
                ...
                cursor.execute(
                    'UPDATE publisher SET name = %s, status = %s WHERE publisherid =% s', (name, status, (publisherid, ), ))
                mysql.connection.commit()
            else:
                cursor.execute(
                    'INSERT INTO publisher (`name`, `status`) VALUES (%s, %s)', (name, status))
                mysql.connection.commit()
...
def editPublisher():
    if 'loggedin' in session:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'SELECT publisherid, name, status FROM publisher WHERE publisherid = %s', (publisherid,))
        publishers = cursor.fetchall()
...
def delete_publisher():
    if 'loggedin' in session:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'DELETE FROM publisher WHERE publisherid = % s', (publisherid, ))
        mysql.connection.commit()
        return redirect(url_for('publisher'))
    return redirect(url_for('login'))
    ...
    if 'loggedin' in session:
        ...
        if request.method == 'POST' and 'name' in request.form and 'status' in request.form:
            ...
            if action == 'updateRack':
                ...
                cursor.execute(
                    'UPDATE rack SET name = %s, status = %s WHERE rackid =% s', (name, status, (rackid, ), ))
                mysql.connection.commit()
            else:
                cursor.execute(
                    'INSERT INTO rack (`name`, `status`) VALUES (%s, %s)', (name, status))
                mysql.connection.commit()
...
def editRack():
    if 'loggedin' in session:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'SELECT rackid, name, status FROM rack WHERE rackid = %s', (rackid,))
        racks = cursor.fetchall()
